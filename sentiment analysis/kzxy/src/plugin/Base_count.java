...
public class Base_count implements PluginInterface {
	public void Base_count() {
	...
	public void exect(List<Article> allArticle, GlobalData allData) throws PluginExectExcept {
		...
		XlsWriter writer;
		try {
			writer = new XlsWriter("./xls/base.xls");
		...
		try {
			Process p = Runtime.getRuntime().exec("python ./plugin/base_count/draw.py");
