...
def load_info(urlcol=2, soucol=0, timecol=1, fname=r'./xls/base.xlsx', sheetindex=0):
    '''
...
def arti_areacount(fname, sheetindex, timecol, areacol):
    ...
    for row in xrange(table.nrows):
        if table.cell(row, areacol).value is '' or table.cell(row, timecol).value is '':
            continue
        tatup = (str(int(table.cell(row, timecol).value)), int(table.cell(row, areacol).value))
...
def draw_line(table):
        ...
        '''read data'''
        abroad_data=table.col_values(1)
        native_data=table.col_values(2)
        gangao_data=table.col_values(3)
        '''  remove title'''
        time.pop(0)
        abroad_data.pop(0)
        native_data.pop(0)
